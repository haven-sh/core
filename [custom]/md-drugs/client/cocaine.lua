...
for k, v in pairs(locations.MakePowder) do
    ps.boxTarget('cocaplant' .. k, v.loc, {
        length = v.l,
        width = v.w,
        height = 1.0,
        rotation = v.rot
        ...
        action = function()
            ...
            if not ps.progressbar(ps.lang('coke.makePow'), 4000, 'uncuff') then
                return
            end
...
for k, v in pairs(locations.CuttingCoke) do
    ps.boxTarget('cutcoke' .. k, v.loc, {
        ...
        canInteract = function()
            if not handleGang(v.gang) then
                return false
            end
            if cuttingcoke ~= nil or baggingcoke ~= nil then
                return false
            end
            return true
        end
...
for k, v in pairs(locations.cokeTele) do
    ps.boxTarget('coke_tele' .. k, v.inside, {
        length = v.l,
        width = v.w,
        heading = v.rot
    }, {{
        ...
        action = function()
            local exitLocation = v.exitCoord or v.outside
            SetEntityCoords(PlayerPedId(), exitLocation)
        end,
        canInteract = function()
            return handleGang(v.gang)
        end
    ...
    ps.boxTarget('coke_teleout' .. k, v.outside, {
        length = v.l,
        width = v.w,
        heading = v.rot
    }, {{
        ...
        action = function()
            local enterSpawn = v.spawnInside or v.inside
            SetEntityCoords(PlayerPedId(), enterSpawn)
        end,
        canInteract = function()
            return handleGang(v.gang)
        end
...
CreateThread(function()
    ...
    BikerCocaine.Details.Enable({BikerCocaine.Details.cokeBasic1, BikerCocaine.Details.cokeBasic2,
                                 BikerCocaine.Details.cokeBasic3}, true)
end)
